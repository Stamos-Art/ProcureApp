...
def get_top_items_by_spend(award_query, limit=5):
    """Get top items by total spend"""
    item_spend = {}
    for award in award_query.all():
        desc = award.request_item.description if award.request_item else 'Είδος #' + str(award.request_item_id)
        item_spend[desc] = item_spend.get(desc, 0) + float(award.line_total or 0)
    
    return sorted(item_spend.items(), key=lambda x: x[1], reverse=True)[:limit]

def get_budget_utilization(rfq_query):
    """Calculate budget utilization per cost center"""
    utilization = {}
    for rfq in rfq_query.all():
        if not rfq.cost_center: continue
        cc_name = rfq.cost_center.name
        if cc_name not in utilization:
            utilization[cc_name] = {'budget': 0, 'actual': 0}
        utilization[cc_name]['budget'] += float(rfq.estimated_budget or 0)
        
        # Actual is based on awards for this RFQ
        from models import ItemAward
        awards = ItemAward.query.filter_by(request_id=rfq.id).all()
        utilization[cc_name]['actual'] += sum(float(aw.line_total or 0) for aw in awards)
