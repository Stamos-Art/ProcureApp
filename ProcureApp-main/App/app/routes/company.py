...
def _check_and_update_rfq_approval_status(rfq):
    """
    Check if the current total awarded value exceeds the user's approval limit.
    If yes, move to PENDING_FINAL_APPROVAL. If no, stay in current state or move to CLOSED.
    NOTE: This only transitions TO PENDING_FINAL_APPROVAL if role is 'company'.
    """
    from decimal import Decimal
    
    # Calculate current grand total awarded
    awards = ItemAward.query.filter_by(request_id=rfq.id).all()
    ...
    if session.get('role') == 'company' and grand_total_awarded > user_limit:
        if rfq.status != RFQStatus.PENDING_FINAL_APPROVAL:
            update_rfq_status(rfq, RFQStatus.PENDING_FINAL_APPROVAL, session.get('role'))
            log_action(rfq.id, f"Αναμονή οικονομικής έγκρισης (υπερβαίνει όριο: {user_limit}€). Σύνολο: {grand_total_awarded}€")
            notify_role('chief', f"Η ζήτηση #{rfq.id} απαιτεί οικονομική έγκριση.", 
                       url_for('company.request_detail', req_id=rfq.id))
            return True, f"Η ανάθεση υπερβαίνει το όριο σας ({user_limit}€). Απεστάλθη για έγκριση."
...
def dashboard():
    ...
    for r in all_rfqs_unfiltered:
        # 1. Pending Approvals (For Chief)
        if user_role == 'chief' and r.status in [RFQStatus.PENDING, RFQStatus.PENDING_FINAL_APPROVAL]:
            command_center['pending_my_approval'].append(r)
            
        # 2. Awaiting Award (Needs evaluation by creator)
        if r.status == RFQStatus.OPEN and len(r.bids) > 0 and r.created_by == user_name:
            command_center['awaiting_award'].append(r)
            
        # 3. Awaiting Delivery (Awarded but not received)
        if r.status == RFQStatus.CLOSED and r.created_by == user_name:
            command_center['awaiting_delivery'].append(r)
            
        # 4. Attention Needed (Deadlines, Low Bids)
        if r.status == RFQStatus.OPEN and r.created_by == user_name:
            if r.submit_deadline and r.submit_deadline < datetime.utcnow():
                command_center['attention_needed'].append({'rfq': r, 'reason': 'Λήξη προθεσμίας - Απαιτείται αξιολόγηση'})
            elif len(r.bids) < 2 and r.submit_deadline and (r.submit_deadline - datetime.utcnow()).days < 2:
                command_center['attention_needed'].append({'rfq': r, 'reason': 'Χαμηλή συμμετοχή προμηθευτών'})
...
def request_detail(req_id):
    ...
    # Calculate lowest line totals for highlighting (after line discounts)
    lowest_line_totals = {}
    for item in items:
        item_totals = []
        for b in bids:
            for bl in b.lines:
                if bl.request_item_id == item.id and bl.line_total is not None:
                    item_totals.append(bl.line_total)
        if item_totals:
            lowest_line_totals[item.id] = min(item_totals)
    ...
    return render_template("company_request_detail.html", 
                           rfq=rfq, items=items, bids=bids, awards=awards, lowest_line_totals=lowest_line_totals, 
...
def award_bids(req_id):
    ...
    # Check approval limit
    over_limit, msg = _check_and_update_rfq_approval_status(rfq)
...
def accept_bid(req_id, bid_id):
    ...
    # Validate RFQ is in bidding phase or returned for revision
    if rfq.status not in [RFQStatus.OPEN, RFQStatus.PENDING_FINAL_APPROVAL, RFQStatus.RETURNED_FOR_REVISION]:
...
@company_bp.route("/requests/<int:req_id>/bids/<int:bid_id>/accept_only", methods=["POST"])
@require_roles("company", "chief")
def accept_bid_only(req_id, bid_id):
    """Mark a bid as accepted (candidate) without awarding items yet"""
    rfq = RequestRFQ.query.get_or_404(req_id)
    bid = Bid.query.get_or_404(bid_id)
    ...
    log_action(req_id, f"Η προσφορά #{bid_id} από {bid.supplier_name} σημειώθηκε ως αποδεκτή.")
    db.session.commit()
    flash(f"Η προσφορά του {bid.supplier_name} σημειώθηκε ως αποδεκτή. Μπορείτε τώρα να προχωρήσετε σε μερική ή συνολική ανάθεση.", "success")
    return redirect(url_for('company.request_detail', req_id=req_id))
...
def award_item_ajax(req_id):
    ...
    if request_item_id == 'shipping':
        ...
        response_msg = "Τα μεταφορικά ανατέθηκαν."
        if over_limit:
            response_msg = msg
            
        return jsonify({"success": True, "message": response_msg, "over_limit": over_limit})
    ...
    response_msg = "Το είδος ανατέθηκε."
    if over_limit:
        response_msg = msg
...
def award_all_from_bid(req_id, bid_id):
    ...
    # 2. Check approval limit and commit
    over_limit, msg = _check_and_update_rfq_approval_status(rfq)
...
def finalize_award(req_id):
    ...
    update_rfq_status(rfq, RFQStatus.CLOSED, session.get('role'))
    rfq.award_date = datetime.utcnow()
    log_action(req_id, "Οριστική κατακύρωση παραγγελίας.")
...
@company_bp.route("/requests/<int:req_id>/save_receipt", methods=["GET", "POST"])
@require_roles("company", "chief")
def save_receipt(req_id):
    ...
    # Use the same phase detection logic as the UI for consistency
    from app.auth import get_phase_key
    phase = get_phase_key(rfq)
    
    if phase not in ['pending_delivery', 'received']:
        flash(f"Η ζήτηση πρέπει να βρίσκεται στο στάδιο 'Αναμένει παράδοση' (τρέχουσα φάση: {phase}) για να καταχωρήσετε ποσότητες παραλαβής.", "warning")
        return redirect(url_for('company.request_detail', req_id=rfq.id))
        
    for item in rfq.items:
        recv_qty_str = request.form.get(f"recv_qty_{item.id}")
        if recv_qty_str is not None:
            try:
                qty = Decimal(recv_qty_str.strip()) if recv_qty_str.strip() else Decimal(0)
                
                # Validation: Cannot receive more than awarded
                award = ItemAward.query.filter_by(request_id=rfq.id, request_item_id=item.id).first()
                if award and qty > award.qty:
                    flash(f"Η ποσότητα παραλαβής για το είδος '{item.description}' ({qty}) υπερβαίνει την παραγγελία ({award.qty}).", "warning")
                    qty = award.qty
    ...
    # Finalize Receipt logic
    if request.form.get("finalize_receipt") == "1":
        # Check if all items are fully received (for logging/backorder detection)
        awards = ItemAward.query.filter_by(request_id=rfq.id).all()
        receipts = ItemReceipt.query.filter_by(request_id=rfq.id).all()
        receipt_map = {r.request_item_id: r.received_qty for r in receipts}
        
        fully_received = True
        for aw in awards:
            if aw.request_item_id:
                received = receipt_map.get(aw.request_item_id, Decimal(0))
                if received < aw.qty:
        ...
        from app.services.status_service import update_rfq_status
        result = update_rfq_status(rfq, RFQStatus.RECEIVED, session.get('role'))
        
        if result.get('success'):
            if not fully_received:
                log_action(rfq.id, "Ολοκλήρωση παραλαβής (Μερική Παραλαβή / Backorder).")
                flash("Η παραλαβή ολοκληρώθηκε με ελλείψεις (Backorder).", "info")
            else:
                log_action(rfq.id, "Ολοκλήρωση παραλαβής (Πλήρης).")
                flash("Η παραλαβή ολοκληρώθηκε επιτυχώς!", "success")
            
            db.session.commit() # Save the status change
        else:
            flash(f"Σφάλμα κατά την αλλαγή κατάστασης: {result.get('message')}", "danger")
    else:
        # Just saving quantities
        log_action(rfq.id, "Προσωρινή αποθήκευση ποσοτήτων παραλαβής.")
        db.session.commit() # Ensure quantities are saved even if not finalizing
        flash("Οι ποσότητες αποθηκεύτηκαν.", "success")
...
@company_bp.route("/requests/<int:req_id>/po")
@require_roles("company", "chief")
def view_po(req_id):
    """View/Print Purchase Order for a closed RFQ"""
    rfq = RequestRFQ.query.get_or_404(req_id)
    
    # Check if RFQ is in a stage that has a PO (CLOSED or RECEIVED)
    from app.auth import get_phase_key
    phase = get_phase_key(rfq)
    if phase not in ['pending_delivery', 'received']:
        flash("Το δελτίο παραγγελίας είναι διαθέσιμο μόνο μετά την οριστική κατακύρωση.", "warning")
        return redirect(url_for('company.request_detail', req_id=rfq.id))
    
    # Get all awards for this RFQ
    awards = ItemAward.query.filter_by(request_id=req_id).options(joinedload(ItemAward.request_item)).all()
    ...
    for aw in awards:
        s_name = aw.supplier_name
        if s_name not in suppliers_data:
            suppliers_data[s_name] = {
                'awarded_items': [],
                'total': Decimal(0)
            }
        
        suppliers_data[s_name]['awarded_items'].append(aw)
        suppliers_data[s_name]['total'] += aw.line_total
        grand_total += aw.line_total
        
    return render_template("po_view.html", 
                           rfq=rfq, 
