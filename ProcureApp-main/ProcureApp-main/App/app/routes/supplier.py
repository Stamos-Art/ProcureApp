...
def dashboard():
    ...
    status_filters = [
        ...
        if v in {'open', 'submitted', 'accepted', 'awarded', 'received', 'rejected', 'reopen', 'no_bid', 'draft', 'declined'}
    ]
    ...
    stats_pending_eval = len([b for b in supplier_bids if b.status == BidStatus.SUBMITTED])
    
    performance_stats = {
        'total_bids': stats_total_bids,
        'won_bids': stats_won_bids,
        'pending_eval': stats_pending_eval,
...
def bid(req_id):
    ...
    if rfq.status in [RFQStatus.CLOSED, RFQStatus.RECEIVED] and existing_bid:
        my_awards = ItemAward.query.filter(
            ItemAward.request_id == req_id,
            or_(
                ItemAward.supplier_id == supplier_user.id,
                ItemAward.supplier_name == supplier_user.display_name
            )
        ).all()
    ...
    # Count only actual items (exclude shipping/combo awards with no request_item_id)
    real_items_count = len([aw for aw in my_awards if aw.request_item_id])
    
    receipt_summary = {
        'total_items': real_items_count,
        'fully_received': 0,
...
@supplier_bp.route("/requests/<int:req_id>/po")
@require_role("supplier")
def view_po(req_id):
    """View/Print Purchase Order for the portion awarded to this supplier"""
    from models import User
    rfq = RequestRFQ.query.get_or_404(req_id)
    username = session.get('username')
    supplier_user = User.query.filter_by(username=username).first()
    ...
    # Check if RFQ is in a stage that has a PO
    from app.auth import get_phase_key
    phase = get_phase_key(rfq)
    if phase not in ['pending_delivery', 'received']:
        flash("Το δελτίο παραγγελίας δεν είναι ακόμα διαθέσιμο.", "warning")
        return redirect(url_for('supplier.bid', req_id=rfq.id))
    
    # Get ONLY awards for THIS supplier
    # Use both ID and Name for robustness with older records
    awards = ItemAward.query.filter(
        ItemAward.request_id == req_id,
        or_(
            ItemAward.supplier_id == supplier_user.id,
            ItemAward.supplier_name == supplier_user.display_name
        )
    ).options(db.joinedload(ItemAward.request_item)).all()
    
    if not awards:
        flash("Δεν βρέθηκαν αναθέσεις για εσάς σε αυτή τη ζήτηση.", "warning")
        return redirect(url_for('supplier.bid', req_id=rfq.id))
    
    # Group awards by supplier (just one for the supplier view)
    suppliers_data = {
        supplier_user.display_name: {
            'awarded_items': awards,
            'total': sum((aw.line_total or Decimal(0)) for aw in awards)
        }
    }
    grand_total = suppliers_data[supplier_user.display_name]['total']
